              ...
              <form onSubmit={handleSubmit} className="space-y-6">
                    ...
                    {budgetOptions.map((option, i) => (
                      <label key={i} className="flex items-center space-x-2 cursor-pointer">
                        <input
                          ...
                          required={i === 0}
                        />
                        <div className={`flex-1 px-3 py-2 text-sm rounded-lg text-center transition-all ${form.budget === option ? 'bg-blue-500/20 border-blue-500' : 'bg-white/5 border border-white/10 hover:border-white/20'}`}>
                          {option}
                        </div>
                      </label>
                    ))}
                ...
                {status && (
                  <motion.div
                    ...
                    animate={{ opacity: 1, y: 0 }}
                    className={`p-4 rounded-xl ${status.includes("successfully") ? 'bg-green-500/10 border-green-500/20' : 'bg-blue-500/10 border-blue-500/20'} border text-center`}
                  >
                    <div className="flex items-center justify-center gap-2">
                      {status.includes("successfully") && <CheckCircle className="w-5 h-5 text-green-400" />}
        ...
        >
          <div className="glass-card p-12 rounded-3xl backdrop-blur-xl border border-white/10 max-w-4xl mx-auto">
            ...
            <div className="flex flex-col sm:flex-row gap-4 justify-center">
              <button
                onClick={() => window.open('https://calendly.com/coderlala/consultation', '_blank')}
              ...
              >
                ...
                <span className="relative">Email Us Directly</span>
              </a>
            </div>
          </div>
        </motion.div>
